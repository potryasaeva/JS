...
let arrayOfElements = document.querySelectorAll('button');
...
function clickHandler(event) {
    ...
    document.body.insertAdjacentHTML('beforeend', showCategory(event.target.dataset.type));
}
...
function showCategory(category) {
    ...
    switch (category) {
        case "phones":
            products.phones.forEach(product => { total += getProductMarkup(product) });
            break;
        case "tablets":
            products.tablets.forEach(product => { total += getProductMarkup(product) });
            break;
        case "tv":
            products.tv.forEach(product => { total += getProductMarkup(product) });
            break;
        default:
            break;
    }
